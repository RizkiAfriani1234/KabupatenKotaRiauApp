package id.ac.polbeng.rizkiafriani.kabupatenkotariauapp
...
import id.ac.polbeng.rizkiafriani.kabupatenkotariauapp.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    //tambahan pada saat diganti emulatornya tidak kembali restart
    override fun onSaveInstanceState(outState: Bundle) {
        super.onSaveInstanceState(outState)
        outState.putString("CURRENT_TITLE", title) // Simpan judul/mode tampilan
    }

    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        super.onRestoreInstanceState(savedInstanceState)
        title = savedInstanceState.getString("CURRENT_TITLE", "Mode List View") // Pulihkan judul
        setActionBarTitle(title)

        // Atur kembali tampilan sesuai dengan mode sebelumnya
        when (title) {
            "Mode List View" -> showRecyclerList()
            "Mode Grid View" -> showRecyclerGrid()
            "Mode Card View" -> showRecyclerCardView()
        }
    }
